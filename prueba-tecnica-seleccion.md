...
## Ejercicio 3 — Explicar una API a un cliente por email

En esta parte te facilitaremos un enlace a la documentación de una API con tres endpoints:

- crear item
- consultar stock
- crear documento

Además, la API expone documentación en `/docs` y `/redoc`.

### Tu objetivo

Queremos ver cómo conviertes una documentación técnica en una explicación clara y útil.

La dificutad es que se lo tienes que explicar a un desarrollador muy capaz pero que sólo conoce su stack (va con una camiseta *"I 💖 COBOL"* ) y nunca se ha interactuado con una API REST, SOAP o ...

Debes redactar un email formativo en el que expliques cómo trabajar con esos endpoints, como probarlos, usarlos, etc. sin entrar en su stack.

### Qué esperamos ver

Contestación por correo, rebosante de paciencia, explicandonos como debo usar la API para que pueda avanzar en la integración de mi ERP con esta API.

El correo interactuar con la API, desde 0. Debe ser fácil de seguir y dejar claro qué tengo que hacer, en qué orden y validar por mi mismo el funcionamiento y avanzar con exito en la integración de la API.

Puedes utilizar pantallazos, apoyarte en la documentación o en cualquier herramienta externa que consideres y me ayude a seguir la explicación.
...
| Comunicación | Trade-offs, dudas y decisiones explicadas de forma honesta, y capacidad de trasladar contenido técnico a una persona cliente con claridad |
